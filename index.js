...
function startCycle() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (parseInt(process.env.AUTOMATIC_SUPPLY_BALANCING_ENABLED))
            yield tryBalanceSupplyAllCurrencies();
...
function tryBalanceSupplyAllCurrencies() {
    return __awaiter(this, void 0, void 0, function* () {
        for (const key of Object.keys(db.currencies))
            if (key !== "ETH")
                yield tryBalanceSupplyByCurrency(key);
    });
}
...
function tryBalanceSupplyByCurrency(currencyCode) {
    return __awaiter(this, void 0, void 0, function* () {
        if (currenciesBalancingSupply[currencyCode])
            return console.warn("Cannot balance supply for", currencyCode, ": supply balancing already in progress");
        currenciesBalancingSupply[currencyCode] = true;
        console.log("Trying to balance supply of", currencyCode);
        // Check withdrawals queue
        var sumPendingWithdrawalsBN = process.env.WITHDRAWAL_QUEUE_ENABLED ? yield getSumPendingWithdrawals(currencyCode) : web3.utils.toBN(0);
        // Get ideal balances
        try {
            var idealBalances = yield getIdealBalances(currencyCode, web3.utils.toBN(0).sub(sumPendingWithdrawalsBN));
        }
        catch (error) {
        ...
        if (sumPendingWithdrawalsBN.gt(web3.utils.toBN(0)))
